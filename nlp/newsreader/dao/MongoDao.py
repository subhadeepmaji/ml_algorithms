...
def build_connections(db_name, alias_name, connect_str):
    if connect_str:
        db_connection = connect(db=db_name, alias=alias_name, host=connect_str)
    else:
        host = MONGO_HOST
        port = MONGO_PORT
        db_connection = connect(db=db_name, alias=alias_name, host=host, port=port)


class NewsArticleDao(object):

    def __init__(self, db_name=DB_NAME, alias_name=ALIAS_NAME, connect_str=None):
